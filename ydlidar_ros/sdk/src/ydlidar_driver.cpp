...
result_t YDlidarDriver::waitPackage(node_info *node, uint32_t timeout) {
  ...
  if (package_Sample_Index == 0) {
    ...
    while ((waitTime = getms() - startTs) <= timeout) {
      ...
      if (!IS_OK(ans)) {
        fprintf(stderr, "[waitPackage] Got an error while waiting for data: %d\n", ans);
        return ans;
      }
    ...
    if (PackagePaidBytes == recvPos) {
      ...
      if (package_Sample_Num * PackageSampleBytes != recvPos) {
        fprintf(stderr, "[waitPackage] Wrong package size\n");
        return RESULT_FAIL;
      }
    } else {
      fprintf(stderr, "[waitPackage] PackagePaidBytes != recvPos\n");
      return RESULT_FAIL;
    }
...
result_t YDlidarDriver::waitScanData(node_info *nodebuffer, size_t &count,
  ...
  if (!isConnected) {
    count = 0;
    fprintf(stderr, "[waitScanData] Not connected\n");
    return RESULT_FAIL;
  }
  ...
  while ((waitTime = getms() - startTs) <= timeout && recvNodeCount < count) {
    ...
    if (!IS_OK(ans)) {
      fprintf(stderr, "[waitScanData] Failed waiting package\n");
      return ans;
    }
  ...
  count = recvNodeCount;
  fprintf(stderr, "[waitScanData] Timeout\n");
  return RESULT_FAIL;
}
